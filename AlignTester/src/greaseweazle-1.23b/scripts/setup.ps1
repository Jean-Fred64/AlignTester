...
$gwSourceDir = Split-Path -Parent $scriptPath
...
if (-not (Test-Path $vsPath)) {
    $vsPath = "${env:ProgramFiles}\Microsoft Visual Studio\2019\BuildTools"
}
if (-not (Test-Path $vsPath)) {
...
& $pythonCmd -m pip install -e . --force-reinstall 2>&1 | Tee-Object -Variable buildOutput
if ($LASTEXITCODE -ne 0) {
    Write-Warn "Échec avec -e, tentative avec installation normale..."
    & $pythonCmd -m pip install . --force-reinstall 2>&1 | Tee-Object -Variable buildOutput
    if ($LASTEXITCODE -ne 0) {
